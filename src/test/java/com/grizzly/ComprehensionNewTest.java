...
class ComprehensionNewTest {
    ...
    @Nested
    @DisplayName("Comprehension validation (evens, labels, dict)")
    class ComprehensionValidationTests {

        @Test
        @DisplayName("[num for num in numbers if num % 2 == 0] yields evens only")
        void listCompWithIfFilter() throws Exception {
            String template = """
                def transform(INPUT):
                    numbers = [1, 2, 3, 4, 5]
                    evens_only = [num for num in numbers if num % 2 == 0]
                    OUTPUT = {}
                    OUTPUT["evens"] = evens_only
                    return OUTPUT
                """;
            GrizzlyTemplate compiled = engine.compile(template);
            Map<String, Object> result = compiled.executeRaw(new HashMap<>());
            @SuppressWarnings("unchecked")
            List<Object> evens = (List<Object>) result.get("evens");
            assertThat(evens).hasSize(2);
            assertThat(((Number) evens.get(0)).intValue()).isEqualTo(2);
            assertThat(((Number) evens.get(1)).intValue()).isEqualTo(4);
        }

        @Test
        @DisplayName("[\"Even\" if num % 2 == 0 else \"Odd\" for num in numbers]")
        void listCompWithTernaryElement() throws Exception {
            String template = """
                def transform(INPUT):
                    numbers = [1, 2, 3, 4, 5]
        ...
        @Test
        @DisplayName("{name: len(name) for name in names} yields name-length dict")
        void dictComprehension() throws Exception {
            String template = """
                def transform(INPUT):
                    names = ["Alice", "Bob", "Charlie"]
                    name_lengths = {name: len(name) for name in names}
                    OUTPUT = {}
                    OUTPUT["lengths"] = name_lengths
                    return OUTPUT
                """;
            GrizzlyTemplate compiled = engine.compile(template);
            Map<String, Object> result = compiled.executeRaw(new HashMap<>());
            @SuppressWarnings("unchecked")
            Map<String, Object> lengths = (Map<String, Object>) result.get("lengths");
            assertThat(lengths).containsEntry("Alice", 5).containsEntry("Bob", 3).containsEntry("Charlie", 7);
        }
    }
}
